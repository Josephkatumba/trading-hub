...
function bucket(trades,key){
  ...
  return [...map.entries()].map(([name,items])=>{
    const pnl=items.reduce((s,t)=>s+(Number(t.pnl)||0),0),wins=items.filter(t=>Number(t.pnl)>0).length;
...
function equityStats(trades){
  ...
  let equity=0,peak=0,maxDrawdown=0,maxDrawdownPct=0;const curve=[];
  ...
  const wins=ordered.filter(t=>Number(t.pnl)>0).map(t=>Number(t.pnl)),losses=ordered.filter(t=>Number(t.pnl)<0).map(t=>Math.abs(Number(t.pnl)));
...
export function analyzeBehavior(trades){
  ...
  const rules=ruleNames.map((name,i)=>{const checked=reviewed.filter(x=>(x.review.rules||[]).includes(i)).map(x=>x.trade);const recorded=reviewed.filter(x=>(x.review.rules||[]).some(r=>r===i)).length;return {name,count:checked.length,recorded,pnl:checked.reduce((s,t)=>s+(Number(t.pnl)||0),0),winRate:checked.length?checked.filter(t=>t.pnl>0).length/checked.length*100:0};});
  const ordered=[...trades].sort((a,b)=>String(a.time).localeCompare(String(b.time)));
  const risk=ordered.map(t=>Number(t.risk)||0).filter(Boolean),riskAvg=avg(risk),riskCv=riskAvg?Math.sqrt(avg(risk.map(x=>(x-riskAvg)**2)))/riskAvg:0;
  ...
  const wins=trades.filter(t=>Number(t.pnl)>0).map(t=>Number(t.pnl)),losses=trades.filter(t=>Number(t.pnl)<0).map(t=>Math.abs(Number(t.pnl)));
  const expectancy=avg(trades.map(t=>Number(t.pnl)||0)),winAvg=avg(wins),lossAvg=avg(losses),payoff=lossAvg?winAvg/lossAvg:null;
  ...
  const sampleScore=Math.min(100,trades.length/50*100),reviewScore=Math.min(100,reviewCoverage),riskScore=Math.min(100,riskCoverage);
  const confidence=Math.round(sampleScore*.5+reviewScore*.25+riskScore*.25);
  const confidenceLabel=confidence>=75?"Strong":confidence>=45?"Developing":"Early";
  ...
  return {reviewedCount:reviewed.length,reviewCoverage,riskAvg,riskCv,riskCoverage,confidence,confidenceLabel,setups,grades,rules,afterWin:{count:afterWin.length,avgPnl:avg(afterWin.map(t=>Number(t.pnl)||0))},afterLoss:{count:afterLoss.length,avgPnl:avg(afterLoss.map(t=>Number(t.pnl)||0))},maxWin,maxLoss,bySymbol,bySession,bySide,byAccount,pair,largestLosses,largestWins,expectancy,winAvg,lossAvg,payoff,medianPnl:median(trades.map(t=>Number(t.pnl)||0)),lossRate,tradeCount:trades.length,wins:wins.length,losses:losses.length,equity,...equity};
}
export function behaviorSummary(b){return {coverage:pct(b.reviewCoverage),riskConsistency:b.riskCv?Math.max(0,Math.min(100,(1-b.riskCv)*100)).toFixed(0):"—",strongestSetup:b.setups[0]?.name||"—",strongestSetupPnl:b.setups[0]?.pnl||0,confidence:b.confidence||0,confidenceLabel:b.confidenceLabel||"Early"};}
