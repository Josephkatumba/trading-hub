...
function delimiterFor(line){
  ...
  return choices.sort((a,b)=>{
    const ac=line.split(a).length, bc=line.split(b).length;
    return bc-ac;
  })[0];
}
...
export function parseCSV(text){
  ...
  const find=(row,names)=>{
    for(const name of names){
      const i=headers.indexOf(cleanHeader(name));
      if(i>=0)return row[i]??"";
    }
    return "";
  };
  return lines.slice(1).map((line,index)=>{
    ...
    return {
      ...
      entry:num(find(row,aliases.entry)),
      exit:num(find(row,aliases.exit)),
      volume:num(find(row,aliases.volume)),
      pnl,
      r:num(find(row,aliases.r)),
      risk:num(find(row,aliases.risk))||Math.abs(pnl),
...
export function calculateMetrics(trades){
  ...
  return {
    ...
    avgR,maxDrawdown,bestInstrument,bestSession,grossProfit,grossLoss,
    equityCurve:ordered.map((t,i)=>({i,pnl:t.pnl,equity:ordered.slice(0,i+1).reduce((s,x)=>s+x.pnl,0)}))
  };
}
...
export function getTradeContext(trade,trades){
  ...
  return {
    symbolTrades:sameSymbol.length,
    symbolWinRate:sameSymbol.length?wins.length/sameSymbol.length*100:0,
    symbolAvgR:sameSymbol.length?sameSymbol.reduce((s,t)=>s+(Number(t.r)||0),0)/sameSymbol.length:0,
    symbolPnl:sameSymbol.reduce((s,t)=>s+t.pnl,0),
    sessionPnl:sameSession.reduce((s,t)=>s+t.pnl,0),
    avgRisk
  };
}
