...
function delimiterFor(line){
  const choices=[",",";","\t"];
  return choices.sort((a,b)=>line.split(b==="\t"?"\t":b).length-line.split(a==="\t"?"\t":a).length)[0];
}
const num=value=>Number(String(value??"").replace(/[,$%]/g,"").replace(/,/g,""))||0;
const aliases={
 account:["account","accountname","accountid","login"],
 symbol:["symbol","instrument","market"],
 side:["side","direction","type","action","ordertype"],
 entry:["entry","entryprice","openprice","open"],
 exit:["exit","exitprice","closeprice","close"],
 volume:["volume","lots","size","quantity"],
 pnl:["pnl","profit","profitloss","netprofit","netpnl"],
 r:["r","rr","riskreward"],
 risk:["risk","riskamount","riskusd"],
 time:["time","datetime","date","closetime","opentime","timestamp"],
 session:["session"]
};
export function parseCSV(text){
  ...
  return lines.slice(1).map((line,index)=>{
    ...
    const rawSide=String(find(row,aliases.side)).toUpperCase();
    const time=find(row,aliases.time);
    const pnl=num(find(row,aliases.pnl));
    return {
      id:"IMP-"+Date.now()+"-"+index,
      account:find(row,aliases.account)||"Imported Account",
      symbol:(find(row,aliases.symbol)||"UNKNOWN").toUpperCase(),
      side:rawSide.includes("SELL")?"SELL":"BUY",
      entry:num(find(row,aliases.entry)), exit:num(find(row,aliases.exit)),
      volume:num(find(row,aliases.volume)), pnl,
      r:num(find(row,aliases.r)), risk:num(find(row,aliases.risk))||Math.abs(pnl),
      time:time||new Date().toISOString().slice(0,16).replace("T"," "),
      session:find(row,aliases.session)||inferSession(time)
    };
...
export function calculateMetrics(trades){
  ...
  const wins=ordered.filter(t=>t.pnl>0), losses=ordered.filter(t=>t.pnl<0);
  const grossProfit=wins.reduce((s,t)=>s+t.pnl,0);
  const grossLoss=Math.abs(losses.reduce((s,t)=>s+t.pnl,0));
  let equity=0,peak=0,maxDrawdown=0;
  for(const t of ordered){equity+=t.pnl;peak=Math.max(peak,equity);maxDrawdown=Math.max(maxDrawdown,peak-equity);}
  const by=(key)=>ordered.reduce((m,t)=>(m[t[key]]=(m[t[key]]||0)+t.pnl,m),{});
  ...
  return {pnl,wins:wins.length,losses:losses.length,winRate:ordered.length?wins.length/ordered.length*100:0,profitFactor:grossLoss?grossProfit/grossLoss:0,avgR,maxDrawdown,bestInstrument,bestSession,grossProfit,grossLoss,equityCurve:ordered.map((t,i)=>({i,pnl:t.pnl,equity:ordered.slice(0,i+1).reduce((s,x)=>s+x.pnl,0)}))};
}
export function getAccounts(trades){
  ...
  trades.forEach(t=>{
    ...
    const a=map.get(t.account);a.pnl+=Number(t.pnl)||0;a.trades++;
  });
  return [...map.values()];
}
export function getTradeContext(trade,trades){
  const sameSymbol=trades.filter(t=>t.symbol===trade.symbol);
  const wins=sameSymbol.filter(t=>t.pnl>0);
  const sameSession=sameSymbol.filter(t=>t.session===trade.session);
  const avgRisk=trades.length?trades.reduce((s,t)=>s+(Number(t.risk)||Math.abs(t.pnl)||0),0)/trades.length:0;
  return {symbolTrades:sameSymbol.length,symbolWinRate:sameSymbol.length?wins.length/sameSymbol.length*100:0,symbolAvgR:sameSymbol.length?sameSymbol.reduce((s,t)=>s+(Number(t.r)||0),0)/sameSymbol.length:0,symbolPnl:sameSymbol.reduce((s,t)=>s+t.pnl,0),sessionPnl:sameSession.reduce((s,t)=>s+t.pnl,0),avgRisk};
}
