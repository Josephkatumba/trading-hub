...
function renderDetail(m){
  ...
  const london=(m.london_high&&m.london_low) ? '<div><span>LONDON RANGE</span><b>'+fmt(m.london_low,2)+' — '+fmt(m.london_high,2)+'</b></div>' : "";
  const key=m.nearest_level ? '<div><span>KEY LEVEL</span><b>'+fmt(m.nearest_level,4)+' · '+esc(m.nearest_level_type||"LEVEL")+'</b></div>' : "";
  const action=String(m.action||"WAIT").toUpperCase();
  const risk=action==="WAIT"?"Confirmation required":(m.stop_loss!=null?"SL "+fmt(m.stop_loss,2):"SL not calculated");
  const reward=m.take_profit!=null?"TP "+fmt(m.take_profit,2):"TP not calculated";
  return '<div class="radar-detail">'
    +'<div class="radar-detail-top"><div><span class="kicker">LIVE MARKET INTELLIGENCE</span><h2>'+esc(m.symbol)+'</h2><p>'+esc(m.stage||"NO SETUP")+' · '+esc(m.session||"Session unknown")+'</p></div><div class="radar-big-score"><strong>'+Number(m.score||0)+'</strong><small>/100</small></div></div>'
    +'<div class="radar-action-card '+actionClass(action)+'"><div class="radar-action-main"><span class="action-icon">'+actionIcon(action)+'</span><div><span>ENGINE VERDICT</span><b>'+esc(action)+'</b></div></div><small>'+esc(m.trigger||"Wait for a clean setup.")+'</small></div>'
    +'<div class="radar-level-strip"><div><span>PRICE</span><b>'+fmt(m.price)+'</b></div><div><span>STOP</span><b>'+risk.replace("SL ","")+'</b></div><div><span>TARGET</span><b>'+reward.replace("TP ","")+'</b></div></div>'
    +'<div class="radar-insight-card"><span>AI MARKET READ</span><p>'+esc(m.insight||m.reason||"Waiting for more market structure.")+'</p></div>'
    ...
    +'<div class="radar-warning"><b>DISCIPLINE GATE</b><span>Potential setup only. Trading Hub observes and explains. Wait for your confirmation rules before executing.</span></div>'
    +'</div>';
}
...
export function renderMarketRadar(){
  return '<section class="page-title radar-title"><div><div class="kicker">TRADING HUB · MARKET COMMAND CENTER</div><h1>Market Radar</h1><p class="sub">One screen for structure, momentum, sessions, confluence and emerging trendline-reversal opportunities.</p></div><div class="radar-engine"><i class="live-dot"></i><span id="radarEngineStatus">CONNECTING ENGINE</span></div></section>'
    ...
    +'<section class="radar-grid"><div class="panel radar-market-panel"><div class="panel-head"><div><span class="kicker">OPPORTUNITY MATRIX</span><h2>Where attention belongs</h2></div><span class="radar-refresh" id="radarUpdated">Waiting…</span></div><div class="radar-legend"><span>PAIR</span><span>PRICE / 24H</span><span>BIAS</span><span>STATE</span><span>SCORE</span></div><div class="radar-table" id="radarTable"></div></div><div class="panel radar-detail-panel" id="radarDetail"></div></section>'
