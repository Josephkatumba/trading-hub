...
function tradeDrawer(t,trades){
 ...
 <div class="drawer-backdrop" id="drawer"><section class="trade-drawer">
  ...
  <div class="trade-facts"><div><span>ENTRY</span><b>${t.entry||"—"}</b></div><div><span>EXIT</span><b>${t.exit||"—"}</b></div><div><span>R MULTIPLE</span><b class="${hasRecordedR(t)?"":"unavailable"}">${hasRecordedR(t)?formatR(Number(t.r)):"Unavailable"}</b></div><div><span>RISK</span><b class="${hasRecordedRisk(t)?"":"unavailable"}">${hasRecordedRisk(t)?money(t.risk):"Unavailable"}</b></div></div>
  <div class="drawer-section"><div class="kicker">YOUR HISTORY · ${esc(t.symbol)}</div><div class="history-strip"><div><b>${c.symbolTrades}</b><span>TRADES</span></div><div><b>${pct(c.symbolWinRate)}</b><span>WIN RATE</span></div><div><b>${formatR(c.symbolAvgR)}</b><span>AVG R</span></div></div></div>
