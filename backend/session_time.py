...
LEGACY  (`legacy_bar_time`): the raw MT5 epoch read as if it were UTC - the
        trendline-first-v3 behaviour (the pre-v4 main._bar_dt, frozen verbatim in
        tests/legacy_session_context.py). MT5 epochs encode the broker's server
        wall clock (New York + 7 h), so the "London session" window this selects
        is 2-3 hours early.
...
Tested in the Phase 9 A/B experiment and adopted as trendline-first-v4
(strategies/trendline.py): production main.session_context now uses the
CORRECTED path; LegacyTrendlineStrategy keeps LEGACY reproducible.
"""
...
def legacy_bar_time(row: dict[str, Any]) -> datetime:
    """trendline-first-v3 (pre-v4 main._bar_dt): the raw epoch read as a UTC instant."""
...
def compute_session_context(rows: Sequence[dict[str, Any]], price: float, now_utc: datetime,
    ...
    # Most recent London session present in the M15 history (unchanged from the v3 production logic).
    london_times = []
    for row in rows:
