...
def session_context(rows: list[dict[str, Any]], price: float) -> dict[str, Any]:
    """Session context for the live trendline version. trendline-first-v4 selects the
    London session through the verified MT5 time basis (TRADING_HUB_MT5_SOURCE_TIMEZONE);
    without a basis the London range is unavailable (fail closed). The pre-v4 logic
    (raw epochs read as UTC) is LegacyTrendlineStrategy / session_time.legacy_bar_time."""
    global _BASIS_WARNED
    basis = os.getenv("TRADING_HUB_MT5_SOURCE_TIMEZONE")
    if not basis and not _BASIS_WARNED:
        _BASIS_WARNED = True
        LOGGER.warning("TRADING_HUB_MT5_SOURCE_TIMEZONE is not set: trendline London session range unavailable")
    return STRATEGIES.get(TRENDLINE).session_context(rows, price, datetime.now(timezone.utc), basis)
